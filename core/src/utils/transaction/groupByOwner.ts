...
import { ObjectChangesByOwner, RtdObjectChangeWithDisplay } from './getObjectChangeSummary';
import { getOwnerType } from './getOwnerType';

const getOwner = (change: RtdObjectChangeWithDisplay) => {
	...
	if ('owner' in change && typeof change.owner === 'object') {
		...
		if ('ConsensusV2' in change.owner) return (change.owner.ConsensusV2 as any).authenticator.SingleOwner;
	}
	return '';
};

export const groupByOwner = (changes: RtdObjectChangeWithDisplay[]) =>
