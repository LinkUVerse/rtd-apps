...
import { RtdValidatorSummary } from 'rtd-typescript/client';
 ...
 */
export function getRefGasPrice(validators?: RtdValidatorSummary[]) {
	...
	const totalStaked = validators.reduce(
		(acc, cur) => acc + BigInt(cur.stakingPoolRtdBalance),
		BigInt(0),
	);
	...
	for (let i = 0; i < sortedByGasPrice.length; i++) {
		...
		const stake = BigInt(validator?.stakingPoolRtdBalance);
