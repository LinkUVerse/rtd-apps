...
import { generateKeypairFromMnemonic, requestRtdFromFaucet } from './utils/localnet';
...
test('request RTD from local faucet', async ({ page, extensionUrl }) => {
	...
	await expect(page.getByText(/RTD Received/i)).toBeVisible({ timeout });
	await expect(page.getByTestId('coin-balance')).not.toHaveText(`${originalBalance}RTD`);
});

test('send 20 RTD to an address', async ({ page, extensionUrl }) => {
	const receivedKeypair = await generateKeypairFromMnemonic(receivedAddressMnemonic.join(' '));
	const receivedAddress = receivedKeypair.getPublicKey().toRtdAddress();

	const originKeypair = await generateKeypairFromMnemonic(currentWalletMnemonic.join(' '));
	const originAddress = originKeypair.getPublicKey().toRtdAddress();

	await importWallet(page, extensionUrl, currentWalletMnemonic);
	await page.getByRole('navigation').getByRole('link', { name: 'Home' }).click();

	await requestRtdFromFaucet(originAddress);
	await expect(page.getByTestId('coin-balance')).not.toHaveText('0RTD');
...
test('check balance changes in Activity', async ({ page, extensionUrl }) => {
	...
	await page
		...
		.first()
		.click();
	await expect(page.getByText(`${COIN_TO_SEND} RTD`, { exact: false })).toBeVisible();
});
