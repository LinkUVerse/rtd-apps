...
export const PASSWORD = 'linkulabs';
...
export async function createWallet(page: Page, extensionUrl: string) {
	...
	await page.getByLabel('Create Account Password').fill('linkulabs');
	await page.getByLabel('Confirm Account Password').fill('linkulabs');
	...
	await page.getByRole('link', { name: /Open Rtd Wallet/ }).click();
}
