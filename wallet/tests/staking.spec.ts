...
test('staking', async ({ page, extensionUrl }) => {
	...
	await page.getByText(/Stake and Earn RTD/).click();
	...
	await expect(page.getByTestId(`stake-button-${STAKE_AMOUNT}-RTD`)).toBeVisible({
		timeout: TEST_TIMEOUT,
	});
	await page.getByTestId(`stake-button-${STAKE_AMOUNT}-RTD`).click();
