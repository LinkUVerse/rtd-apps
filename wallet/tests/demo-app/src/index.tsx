...
import { type RtdWallet } from '_src/dapp-interface/WalletStandardInterface';
import { Transaction } from 'rtd-typescript/transactions';
import { getWallets, ReadonlyWalletAccount, type Wallet } from 'rtd-wallet-standard';
...
function findRtdWallet(wallets: readonly Wallet[]) {
	return (wallets.find((aWallet) => aWallet.name.includes('Rtd Wallet')) ||
		null) as RtdWallet | null;
}

function App() {
	const [rtdWallet, setRtdWallet] = useState<RtdWallet | null>(() =>
		findRtdWallet(getWallets().get()),
	);
	...
	useEffect(() => {
		...
		function updateWallets() {
			setRtdWallet(findRtdWallet(walletsApi.get()));
		}
	...
	useEffect(() => {
		if (rtdWallet) {
			return rtdWallet.features['standard:events'].on('change', ({ accounts }) => {
				if (accounts) {
					setAccounts(rtdWallet.accounts);
				}
			});
		}
	}, [rtdWallet]);
	if (!rtdWallet) {
		return <h1>Rtd Wallet not found</h1>;
	}
	return (
		<>
			<h1>Rtd Wallet is installed. ({rtdWallet.name})</h1>
			{accounts.length ? (
				...
				<button onClick={async () => rtdWallet.features['standard:connect'].connect()}>
					Connect
				</button>
			)}
			<label>
			...
			<button
				onClick={async () => {
					...
					const txb = getDemoTransaction(accounts[0]?.address || '0x01');
					try {
						await rtdWallet.features['rtd:signTransactionBlock']!.signTransactionBlock({
							transactionBlock: txb,
							account: getAccount(accounts[0], useWrongAccounts),
							chain: 'rtd:unknown',
						});
			...
			<button
				onClick={async () => {
					setError(null);
					try {
						await rtdWallet.features['rtd:signMessage']?.signMessage({
							account: getAccount(accounts[0], useWrongAccounts),
