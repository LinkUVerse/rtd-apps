...
export function TokenRow({
	...
	return (
		<Tag
			className={clsx(
				'group flex py-3 pl-1.5 pr-2 rounded hover:bg-rtd/10 items-center bg-transparent border-transparent',
...
function TokenDetails({ coinType }: TokenDetailsProps) {
	...
	const domainName = useResolveRtdNSName(activeAccountAddress);
	...
	const { data } = useQuery({
		...
		staleTime: 2 * 60 * 1000,
		retry: false,
		enabled: false, // Temporarily disabled due to API endpoint issues
	});
							...
							<>
								...
								>
									...
									<div className="w-full">
										{accountHasRtd || delegatedStake?.length ? (
											<TokenIconLink
												disabled={!tokenBalance}
