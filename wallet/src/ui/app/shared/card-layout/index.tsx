...
import { Rtd, ThumbUpFill32 } from 'rtd-apps-icons';
...
export function CardLayout({ children, title, subtitle, headerCaption, icon }: CardLayoutProps) {
	return (
		<div className="flex flex-col flex-nowrap rounded-20 items-center bg-rtd-lightest shadow-wallet-content p-7.5 pt-10 flex-grow w-full max-h-popup-height max-w-popup-width overflow-auto">
			{icon === 'success' ? (
			...
			{icon === 'rtd' ? (
				<div className="flex flex-col flex-nowrap items-center justify-center rounded-full w-16 h-16 bg-rtd mb-7">
					<Rtd className="text-white text-4xl" />
				</div>
			) : null}
			{headerCaption ? (
