...
import { getTokenStakeRtdForValidator } from '../getTokenStakeRtdForValidator';
...
export function ValidatorFormDetail({ validatorAddress, unstake }: ValidatorFormDetailProps) {
	...
	const totalValidatorStake = validatorData?.stakingPoolRtdBalance || 0;

	const totalStake = useMemo(() => {
		if (!stakeData) return 0n;
		return unstake
			? getStakeRtdByRtdId(stakeData, stakeIdParams)
			: getTokenStakeRtdForValidator(stakeData, validatorAddress);
	...
	const totalStakePercentage = useMemo(() => {
		...
		return calculateStakeShare(
			BigInt(validatorData.stakingPoolRtdBalance),
