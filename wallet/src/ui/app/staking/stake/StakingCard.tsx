...
import { Coin } from '_redux/slices/rtd-objects/Coin';
...
import { useCoinMetadata, useGetDelegatedStake } from 'rtd-apps-core';
import { useRtdClientQuery } from 'rtd-dapp-kit';
import { ArrowLeft16 } from 'rtd-apps-icons';
...
function StakingCard() {
	...
	const { data: rtdBalance, isPending: loadingRtdBalances } = useRtdClientQuery(
		'getBalance',
		{ coinType: RTD_TYPE_ARG, owner: accountAddress! },
		{ refetchInterval, staleTime, enabled: !!accountAddress },
	);
	const coinBalance = BigInt(rtdBalance?.totalBalance || 0);
	const [searchParams] = useSearchParams();
	const validatorAddress = searchParams.get('address');
	const stakeRtdIdParams = searchParams.get('staked');
	...
	const totalTokenBalance = useMemo(() => {
		...
		return getStakeRtdByRtdId(allDelegation, stakeRtdIdParams);
	}, [allDelegation, stakeRtdIdParams]);

	const stakeData = useMemo(() => {
		if (!allDelegation || !stakeRtdIdParams) return null;
	...
	const stakeToken = useMutation({
		...
		onSuccess: (_, { amount, validatorAddress }) => {
			ampli.stakedRtd({
				stakedAmount: Number(amount / MIST_PER_RTD),
	...
	const unStakeToken = useMutation({
		mutationFn: async ({ stakedRtdId }: { stakedRtdId: string }) => {
			if (!stakedRtdId || !signer) {
				throw new Error('Failed, missing required field.');
			}
	...
	return (
		<div className="flex flex-col flex-nowrap flex-grow w-full">
			<Loading loading={isPending || validatorsisPending || loadingRtdBalances}>
				<Formik
