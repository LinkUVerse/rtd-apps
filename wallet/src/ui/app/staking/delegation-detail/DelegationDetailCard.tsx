...
import type { StakeObject } from 'rtd-typescript/client';
import { MIST_PER_RTD, RTD_TYPE_ARG } from 'rtd-typescript/utils';
...
export function DelegationDetailCard({ validatorAddress, stakedId }: DelegationDetailCardProps) {
	...
	const { data: rtdCoinBalance } = useRtdClientQuery(
		'getBalance',
		{ coinType: RTD_TYPE_ARG, owner: accountAddress!! },
		{ refetchInterval, staleTime, enabled: !!accountAddress },
	);
	const { data: metadata } = useCoinMetadata(RTD_TYPE_ARG);
	// set minimum stake amount to 1 RTD
	const showRequestMoreRtdToken = useMemo(() => {
		if (!rtdCoinBalance?.totalBalance || !metadata?.decimals || apiEnv === API_ENV.mainnet)
			return false;
		const currentBalance = new BigNumber(rtdCoinBalance.totalBalance);
		const minStakeAmount = new BigNumber(MIN_NUMBER_RTD_TO_STAKE).shiftedBy(metadata.decimals);
		return currentBalance.lt(minStakeAmount.toString());
	}, [apiEnv, metadata?.decimals, rtdCoinBalance?.totalBalance]);

	const { data: rollingAverageApys } = useGetValidatorsApy();

	const validatorData = useMemo(() => {
		if (!system) return null;
		return system.activeValidators.find((av) => av.rtdAddress === validatorAddress);
	...
	const delegationId = delegationData?.status === 'Active' && delegationData?.stakedRtdId;
						...
						<div className="flex gap-2.5 w-full my-3.75">
							{!hasInactiveValidatorDelegation ? (
								<Button
									...
									disabled={showRequestMoreRtdToken}
								/>
							) : null}
							...
							{Boolean(totalStake) && delegationId && (
								<Button
									...
									onClick={() => {
										ampli.clickedUnstakeRtd({
											stakedAmount: Number(totalStake / MIST_PER_RTD),
											validatorAddress,
										});
									}}
									text="Unstake RTD"
