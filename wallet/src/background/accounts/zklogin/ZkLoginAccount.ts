...
import { getRtdClient } from '_src/shared/rtd-client';
import { fromExportedKeypair } from '_src/shared/utils/from-exported-keypair';
import { toSerializedSignature, type PublicKey } from 'rtd-typescript/cryptography';
...
async function hasTransactionHistory(address: string): Promise<boolean> {
	const rpc = getRtdClient({ env: API_ENV.mainnet, customRpcUrl: null });
	...
	implements SigningAccount
{
	...
	static async createNew({
		...
		if (normalizeRtdAddress(legacyAddress) !== normalizeRtdAddress(nonLegacyAddress)) {
