...
import type { RtdSignTransactionSerialized } from '_payloads/transactions/ExecuteTransactionRequest';
...
class Transactions {
	...
	public async executeOrSignTransaction(
		{
			...
			| {
					tx?: undefined;
					sign: RtdSignTransactionSerialized;
			  },
		connection: ContentScriptConnection,
	): Promise<RtdTransactionBlockResponse | SignedTransaction> {
	...
	public async getTransactionRequests(): Promise<Record<string, ApprovalRequest>> {
		return (await Browser.storage.local.get({ [TX_STORE_KEY]: {} }))[TX_STORE_KEY] as Record<string, ApprovalRequest>;
	}
