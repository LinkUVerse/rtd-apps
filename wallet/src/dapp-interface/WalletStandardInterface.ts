...
import { isTransaction } from 'rtd-typescript/transactions';
...
import {
	...
	type RtdFeatures,
	type RtdSignAndExecuteTransactionBlockMethod,
	type RtdSignAndExecuteTransactionMethod,
	type RtdSignMessageMethod,
	type RtdSignPersonalMessageMethod,
	type RtdSignTransactionBlockMethod,
	type RtdSignTransactionMethod,
	type Wallet,
...
const name = process.env.APP_NAME || 'Rtd Wallet';
...
const API_ENV_TO_CHAIN: Record<Exclude<API_ENV, API_ENV.customRPC>, ChainType> = {
	[API_ENV.local]: RTD_LOCALNET_CHAIN,
	[API_ENV.devNet]: RTD_DEVNET_CHAIN,
	[API_ENV.testNet]: RTD_TESTNET_CHAIN,
	[API_ENV.mainnet]: RTD_MAINNET_CHAIN,
};

export class RtdWallet implements Wallet {
		...
		this.#accounts = accounts.map(
			({ address, publicKey, nickname }) =>
				new ReadonlyWalletAccount({
					...
					features: ['rtd:signAndExecuteTransaction'],
				}),
		);
	}
	...
	constructor() {
		this.#events = mitt();
		this.#accounts = [];
		this.#messagesStream = new WindowMessageStream('rtd_in-page', 'rtd_content-script');
	...
	#signTransaction: RtdSignTransactionMethod = async ({ transaction, account, ...input }) => {
		return mapToPromise(
			this.#send<SignTransactionRequest, SignTransactionResponse>({
				type: 'sign-transaction-request',
				transaction: {
	...
	#signAndExecuteTransactionBlock: RtdSignAndExecuteTransactionBlockMethod = async (input) => {
	...
	#signAndExecuteTransaction: RtdSignAndExecuteTransactionMethod = async (input) => {
		return mapToPromise(
			this.#send<ExecuteTransactionRequest, ExecuteTransactionResponse>({
				type: 'execute-transaction-request',
				transaction: {
	...
	#signMessage: RtdSignMessageMethod = async ({ message, account }) => {
		return mapToPromise(
	...
	#signPersonalMessage: RtdSignPersonalMessageMethod = async ({ message, account }) => {
		return mapToPromise(
			this.#send<SignMessageRequest, SignMessageRequest>({
				type: 'sign-message-request',
				args: {
	...
	#setActiveChain({ env }: NetworkEnvType) {
		this.#activeChain = env === API_ENV.customRPC ? 'rtd:unknown' : API_ENV_TO_CHAIN[env];
	}
