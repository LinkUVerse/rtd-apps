...
import { Search24 } from "rtd-apps-icons";
import { type RtdMoveNormalizedStruct, type RtdObjectResponse } from "rtd-typescript/client";
import { Text, LoadingIndicator, Combobox, ComboboxInput, ComboboxList } from "@linku/ui";
...
type ObjectFieldsProps = {
	id: string;
	normalizedStructData?: RtdMoveNormalizedStruct;
	rtdObjectResponseData?: RtdObjectResponse;
	loading: boolean;
	error: boolean;
	objectType?: string;
};
...
export function ObjectFieldsCard({
	...
	const fieldsData =
		rtdObjectResponseData?.data?.content?.dataType === "moveObject"
			? (rtdObjectResponseData?.data?.content?.fields as Record<string, string | number | object>)
			: null;
				...
				>
					{normalizedStructData?.fields?.map(({ name, type }) => (
						<button
							type="button"
							key={name}
							className="mt-0.5 rounded-lg border border-transparent p-2.5 hover:border-rtd-primaryBlue2023/20 hover:bg-white/60"
