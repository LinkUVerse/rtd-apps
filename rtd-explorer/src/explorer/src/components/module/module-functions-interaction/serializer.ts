import { RtdMoveNormalizedType } from "rtd-typescript/client";
import { CallArg } from "rtd-typescript/transactions";
import {
    MOVE_STDLIB_ADDRESS,
...
): SerializationTypesAndValue
{
    ...
    if (isPrimitiveType(normalizedType))
    {
        ...
        if (normalizedType === "Address") {
            const argAddr: string | undefined =
                argVal === undefined ? undefined : normalizeRtdAddress(String(argVal).trim());
            if (
                ( !["string", "undefined"].includes(typeof argVal) ) ||
                ( argVal !== undefined && !isValidRtdAddress(argAddr!) )
            ) {
                throw new Error(`Invalid Rtd address: ${JSON.stringify(argVal)}`);
            }
            return {
                types: [normalizedType],
                value: argAddr,
    ...
    if ("Struct" in normalizedType)
    {
        ...
        else if (isSameStruct(normalizedType.Struct, RESOLVED_RTD_ID)) {
    ...
    if ("Vector" in normalizedType)
    {
        ...
        let innerValues: RtdJsonValue[] | undefined;
...
type SerializationTypesAndValue = {
    ...
    value: RtdJsonValue | undefined;
};

type RtdJsonValue = boolean | number | string | CallArg | RtdJsonValue[];
...
function parseTypeArgument(input: string): RtdMoveNormalizedType {
    input = input.trim();
...
function parseTypeArguments(input: string): RtdMoveNormalizedType[] {
    const typeArguments: RtdMoveNormalizedType[] = [];
    let depth = 0;
    let currentArg = "";
