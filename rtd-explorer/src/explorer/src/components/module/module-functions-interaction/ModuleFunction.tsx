...
import { ArrowRight12 } from "rtd-apps-icons";
import { bcs } from "rtd-typescript/bcs";
import type { RtdMoveNormalizedFunction } from "rtd-typescript/client";
...
export type ModuleFunctionProps = {
    ...
    moduleName: string;
    functionName: string;
    functionDetails: RtdMoveNormalizedFunction;
    defaultOpen?: boolean;
};
    ...
    const execute = useMutation({
        mutationFn: async ({ params, types }: TypeOf<typeof argsSchema>) =>
        {
            ...
            if (functionDetails.return.length > 0)
            {
                ...
                // Transfer all returned objects to the sender // TODO: transfer only rtdtable objects
            ...
            const resp = await rtdClient.executeTransactionBlock({
                    ...
                    <ConnectButton
                        ...
                        className={clsx(
                            ...
                            currentAccount
                                ...
                                : "!flex !flex-nowrap !items-center !gap-1 !bg-rtd-dark !font-sans !text-rtd-light hover:!bg-rtd-dark hover:!text-white",
                        )}
                    />
                </div>
