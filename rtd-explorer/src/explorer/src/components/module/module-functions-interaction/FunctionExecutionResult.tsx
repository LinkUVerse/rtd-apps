...
import type { RtdTransactionBlockResponse, OwnedObjectRef } from "rtd-typescript/client";
...
type FunctionExecutionResultProps = {
	result: RtdTransactionBlockResponse | null;
	error: string | false;
	onClear: () => void;
};

export function FunctionExecutionResult({ error, result, onClear }: FunctionExecutionResultProps) {
	const adjError = error || (result?.effects?.status.error) || null;
	...
	return (
		<Banner icon={null} fullWidth variant={variant} spacing="lg" onDismiss={onClear}>
			<div className="space-y-4 text-bodySmall">
				...
				<LinkGroup
					title="Created"
					links={(result?.effects?.created?.map(toObjectLink)) || []}
				/>
				<LinkGroup
					title="Updated"
					links={(result?.effects?.mutated?.map(toObjectLink)) || []}
				/>
