...
import { COIN_GECKO_RTD_URL, useRtdCoinData } from "rtd-apps-core";
...
export function RtdTokenCard() {
	const { data } = useRtdCoinData();
	...
	return (
		<ButtonOrLink href={COIN_GECKO_RTD_URL}>
			<Card growOnHover bg="white/80" spacing="lg" height="full">
				<div className="flex items-center gap-2">
					<div className="h-5 w-5 flex-shrink-0 rounded-full bg-rtd p-1">
						<Rtd className="h-full w-full text-white" />
					</div>
					<div className="flex w-full flex-col gap-0.5">
						<Text variant="body/semibold" color="steel-darker">
							1 RTD = {formattedPrice}
						</Text>
