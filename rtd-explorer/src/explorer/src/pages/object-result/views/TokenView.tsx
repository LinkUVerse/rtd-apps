...
function useObjectFieldsCard(id: string) {
	const { data: rtdObjectResponseData, isPending, isError } = useGetObject(id);

	const objectType =
		rtdObjectResponseData?.data?.type ??
		rtdObjectResponseData?.data?.content?.dataType === "package"
			? rtdObjectResponseData.data.type
			: rtdObjectResponseData?.data?.content?.type;
			...
			<TabsContent value={TABS_VALUES.FIELDS}>
				<FieldsContainer>
					<ObjectFieldsCard
						...
						rtdObjectResponseData={rtdObjectResponseData}
						loading={objectFieldsCardLoading}
...
export function TokenView({ data }: { data: RtdObjectResponse }) {
