...
import { CoinFormat, useFormatCoin, useResolveRtdNSName } from "rtd-apps-core";
import { ArrowUpRight16, Info16 } from "rtd-apps-icons";
import { type ObjectOwner, type RtdObjectResponse } from "rtd-typescript/client";
import {
	formatAddress,
	normalizeStructTag,
	parseStructTag,
...
function AddressOwner({ address }: { address: string }) {
	const { data: rtdnsDomainName } = useResolveRtdNSName(address);

	return <AddressLink address={address} label={rtdnsDomainName} />;
}
...
function OwnerCard({
	...
	const [storageRebateFormatted] = useFormatCoin(storageRebate, RTD_TYPE_ARG, CoinFormat.FULL);
