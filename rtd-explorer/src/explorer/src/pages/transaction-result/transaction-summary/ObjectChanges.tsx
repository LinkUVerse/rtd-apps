...
import { ChevronRight12 } from "rtd-apps-icons";
import {
	type RtdObjectChangePublished,
	type RtdObjectChange,
	type DisplayFieldsResponse,
} from "rtd-typescript/client";
import { parseStructTag } from "rtd-typescript/utils";
...
function ObjectChangeEntries({ changeEntries, type, isDisplay }: ObjectChangeEntriesProps) {
	...
	if (type === "published") {
		expandableItems = (changeEntries as RtdObjectChangePublished[]).map(
			({ packageId, modules }) => (
				<ObjectDetailPanel
					key={packageId}
					openInitially={true}
		...
		expandableItems = (changeEntries as RtdObjectChangeWithDisplay[]).map((change) =>
...
function ObjectChangeEntriesCardFooter({
	...
	const { data: rtdnsDomainName } = useResolveRtdNSName(ownerAddress);
	...
	return (
		<div className="flex flex-wrap items-center justify-between">
			...
			{ownerType === "AddressOwner" && (
				<AddressLink label={rtdnsDomainName || undefined} address={ownerAddress} />
			)}
	...
	return (
		<>
			{Object.entries(sortedObjectSummary).map(([type, changes]) => (
				<ObjectChangeEntriesCards key={type} type={type as RtdObjectChangeTypes} data={changes} />
			))}
		</>
	);
}
