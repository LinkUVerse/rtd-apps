...
import { type RtdEvent, type RtdValidatorSummary } from "rtd-typescript/client";
import { Heading, Text } from "@linku/ui";
	...
	return {
		data: [...validators]
			...
			.map((validator) => {
				...
				const totalStake = validator.stakingPoolRtdBalance;
				const img = validator.imageUrl;

				const event = getValidatorMoveEvent(validatorEvents, validator.rtdAddress) as {
					pool_staking_reward?: string;
				};

				const atRiskValidator = atRiskValidators.find(
					([address]) => address === validator.rtdAddress,
				);
				const isAtRisk = !!atRiskValidator;
				const lastReward = event?.pool_staking_reward ?? null;
				const { apy, isApyApproxZero } = rollingAverageApys?.[validator.rtdAddress] ?? {
					apy: null,
				};
		...
		columns: [
			...
			},
			{
				...
				cell: (props: any) => {
					...
					return atRisk !== null ? (
						<Tooltip
							tip="Staked RTD is below the minimum RTD stake threshold to remain a validator."
						>
...
function ValidatorPageResult() {
	const { data, isPending, isSuccess, isError } = useRtdClientQuery("getLatestRtdSystemState");
	...
	const totalStaked = useMemo(() => {
		...
		return validators.reduce((acc, cur) => acc + Number(cur.stakingPoolRtdBalance), 0);
	}, [data]);
							...
							<Card spacing="lg">
								<div className="flex w-full basis-full flex-col gap-8">
									...
									<div className="flex flex-col gap-8 md:flex-row">
										...
										<div className="flex flex-col gap-8">
											<Stats
												label="Total RTD Staked"
												tooltip="The total RTD staked on the network by validators and delegators to validate the network and earn rewards."
