...
import { isRtdNSName, useResolveRtdNSAddress, useResolveRtdNSName } from "rtd-apps-core";
import { Domain32 } from "rtd-apps-icons";
import { LoadingIndicator } from "@linku/ui";
...
function AddressResultPageHeader({ address, loading }: { address: string; loading?: boolean }) {
	const { data: domainName, isLoading } = useResolveRtdNSName(address);
...
function RtdNSAddressResultPageHeader({ name }: { name: string }) {
	const { data: address, isLoading } = useResolveRtdNSAddress(name);
...
function RtdNSAddressResult({ name }: { name: string }) {
	const { isFetched, data } = useResolveRtdNSAddress(name);
...
export default function AddressResultPage() {
	...
	return (
		<PageLayout
			gradient={{
				size: "md",
				content: isRtdNSAddress ? (
					<RtdNSAddressResultPageHeader name={id!} />
				) : (
					<AddressResultPageHeader address={id!} />
				),
			}}
			content={isRtdNSAddress ? <RtdNSAddressResult name={id!} /> : <AddressResult address={id!} />}
		/>
	);
}
