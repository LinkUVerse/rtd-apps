...
import { KIOSK_ITEM, KioskClient, KioskItem, KioskOwnerCap } from "rtd-kiosk";
...
async function getOriginByteKioskContents(address: string, client: RtdClient) {
...
async function getRtdKioskContents(address: string, kioskClient: KioskClient) {
...
export function useGetKioskContents(address?: string | null, disableOriginByteKiosk?: boolean) {
	const { client: rtdClient, network } = useRtdClientContext();
	...
	return useQuery({
		...
		queryFn: async () => {
			const rtdKiosks = await getRtdKioskContents(normalizedAddress, kioskClient);
			const obKiosks = await getOriginByteKioskContents(normalizedAddress, rtdClient);
			return [...rtdKiosks, ...obKiosks];
		},
		select(data) {
